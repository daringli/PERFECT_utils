...
class ProfileNFromPhiEta(Generator):
    ...
    def generate(self,profileList=[]):
        ...
        wantedProfiles = ["T","ddx_T","eta","ddx_eta"]
                
        [T,ddx_T,eta,ddx_eta] = extractProfilesFromList(profileList,wantedProfiles,self.species)
        [Phi,ddx_Phi] = extractProfilesFromList(profileList,["Phi","ddx_Phi"],"none")
        

        n=lambda x: eta(x)*exp(-(self.Z/self.DeltaOver2Omega)*Phi(x)/T(x))
        ddx_n=lambda x: (ddx_eta(x) + (-self.Z/self.DeltaOver2Omega)*(eta(x)/T(x))*(ddx_Phi(x)-Phi(x)*ddx_T(x)/T(x)))*exp((-self.Z/self.DeltaOver2Omega)*Phi(x)/T(x))
    
        p = Profile(n)
        ddx_p = Profile(ddx_n)
        p.profile = self.profile
        p.species = self.species
        p.generator = 'NFromPhiEta'
        p.generator_dict = vars(self)
        ddx_p.profile = "ddx_" + self.profile
        ddx_p.species = self.species
        ddx_p.generator = 'NFromPhiEta'
...
if __name__ == "__main__":
    ...
    genn = ProfileNFromPhiEta(Z,'',DeltaOver2omega)
    (n,ddx_n) = genn.generate(profiles)
    profiles.append(n)
    profiles.append(ddx_n)
