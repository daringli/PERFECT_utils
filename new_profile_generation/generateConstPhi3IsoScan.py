...
def generateConstPhi(XStart=None,XStop=None,width=None,XPedStart=None,XPedStop=None, \
    ...
    Nions = len(ions)
    assert Nions == len(concentrations)
    
    Zis = [speciesZ([ion])[0] for ion in ions]
    for Z in Zis[1:]:
        assert Z == Zis[0]
    Z=Zis[0]
    Zs=Zis + [-1] #add e
    ...
    for i,ion in enumerate(ions):
        TiGens[i] = Profile3Linear(XStart,XStop,ddx_TiCore,ddx_TiSOL,Twidth,XPedStart,XPedStart +Twidth,TiPed,ddx_TiPed,TiLCFS,profile="T",species=ion,kind=TiKind, transWidth = TiTransWidth)
    ...
    for i,ion in enumerate(ions):
        niGens[i] = Profile3Linear(XStart,XStop,ddx_nCore,ddx_nSOL,width,XPedStart,XPedStop,nPed,ddx_nPed,nLCFS,profile="n",species=ion,kind=nKind, transWidth = nTransWidth,scale=concentrations[i])
    ...
    for i,ion in enumerate(ions):
        etaiGens[i] = Profile3Linear(XStart,XStop,ddx_nCore,ddx_nSOL,etawidth,XPedStart,XPedStart + etawidth,etaPed,ddx_etaPed,YLCFS=etaLCFS,profile="eta",species=ion,kind=nKind, transWidth = etaTransWidth,scale=concentrations[i])

    PhiGen = ProfilePhiFromEtaN(Z,useSpecies=ions[0],DeltaOver2omega=DeltaOver2omega)
    etaeGen = ProfileEtaFromPhiN(-1,species="e",DeltaOver2omega=DeltaOver2omega)

    # gridGen = GridFromPedestalBoundaries
    
    generatorList = TiGens + [TeGen] + etaiGens + niGens + [neGen,PhiGen,etaeGen]
