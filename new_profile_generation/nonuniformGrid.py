...
def generate_nonuniform_grid_bezier(s,psiAHat,psi1,psi2,slope12,smoothness):
    ...
    psiN = s #array of uniform grid points
    Npsi = len(psiN)
    psiNMin = psiN[0]
    psiNMax = psiN[-1]
...
def generate_nonuniform_grid_arctanh(s,psiAHat,slope,psi0):
    #Calculates psi(psiN), where psiN is uniform and psi is nonuniform
    # psi = h(psiN)
...
def generate_nonuniform_grid_Int_arctan(s,psiAHat,a,b,c,psiN1,psiN2):
    #Calculates psi(psiN), where psiN is uniform and psi is nonuniform
    # psi = h(psiN)
