...
def data_from_output(output_filename):
    ...
    for i in range(N_simuls):
        ...
        total_mem = sum(mem_lists[i])
        N_nodes = len(mem_lists[i])
        avg_mem = total_mem/N_nodes
        sd_mem = numpy.sqrt(sum((mem_lists[i] - avg_mem)**2)/N_nodes)
        rects = ax.bar(x + i*width, mem_lists[i], width, color=colorlist[i])
        ax.axhline(y = avg_mem, color = colorlist[i], linestyle='solid')
        ax.axhline(y = avg_mem+sd_mem, color = colorlist[i], linestyle='dashed')
        ax.axhline(y = avg_mem-sd_mem, color = colorlist[i], linestyle='dashed')
        ...
        print "total memory usage: "+ str(total_mem)
